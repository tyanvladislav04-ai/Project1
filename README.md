...
11. Функция ```func_csv```, которая считывает финансовые операции из CSV и выдает список словарей с 
транзакциями.


12. Функция ```funs_excel```, которая считывает финансовыу операции из Excel и выдает список словарей 
с транзакциями.

13. Функция ```process_bank_search```, которая принимает список словарей с данными о банковских операциях 
и строку поиска, а возвращает список словарей, у которых в описании есть данная строка.

14. Функция ```process_bank_operations```, которая принимает список словарей с данными о банковских операциях и список 
категорий операций, а возвращает словарь, в котором ключи — это названия категорий, а значения — это количество операций
в каждой категории.
...
```test_empty_excel_file```, ```test_no_matches```, ```test_basic_search```, ```test_empty_data```, 
```test_duplicate_operations``` проверяют работу функций указанных выше с использованием конструкции assert.
